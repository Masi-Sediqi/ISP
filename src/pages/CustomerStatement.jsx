...
function CustomerStatement() {
  ...
  const activities = [
    ...travels.map((record) => ({ id: `travel-${record.id}`, date: record.date, type: "سفر", title: record.travelName, billed: Number(record.fare || 0) - Number(record.discount || 0), paid: Number(record.paidAmount || 0) })),
    ...payments.map((record) => ({ id: `payment-${record.id}`, date: record.date, type: "پرداخت", title: record.description || "پرداخت بدهی", billed: 0, paid: Number(record.amount || 0) })),
