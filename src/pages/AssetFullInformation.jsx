...
function AssetFullInformation() {
    ...
    const purchaseRecordTotalAmount =
        Number(purchaseRecordForm.quantity || 0) *
        Number(purchaseRecordForm.unitPrice || 0);

    const purchaseRecordRemainingAmount = Math.max(
        purchaseRecordTotalAmount -
        Number(purchaseRecordForm.paidAmount || 0),
        0
    );
...
const getMovementRowClass = (movement) => {
  ...
  setForm((previous) => ({
    ...
    const hasDuplicateSerialNumbers = (identityRecords) => {
        const serials = identityRecords
            .map((record) => record.serialNumber.trim().toLowerCase())
            .filter(Boolean);

        return new Set(serials).size !== serials.length;
    };

    const hasExistingSerialNumber = (identityRecords) => {
        const existingSerials = new Set(
            (asset?.identityRecords || [])
                .map((record) =>
                    String(record.serialNumber || "").trim().toLowerCase()
                )
                .filter(Boolean)
        );

        return identityRecords.some((record) =>
            existingSerials.has(record.serialNumber.trim().toLowerCase())
        );
    };
...
const handleEditPurchaseBillImageChange = (event) => {
    ...
    const saveBalance = async (event) => {
        ...
        if (
            isIndividualAsset &&
            hasDuplicateSerialNumbers(identityRecords)
        ) {
            notify(
        ...
        if (
            isIndividualAsset &&
            hasExistingSerialNumber(identityRecords)
        ) {
            notify(
        ...
        if (!Number.isFinite(unitPrice) || unitPrice < 0) {
            notify("Purchase price cannot be negative.", "error");
            return;
        }
   ...
   const savePurchaseRecord = async (event) => {
    ...
    const unitPrice = Number(
        purchaseRecordForm.unitPrice || 0
    );

    const totalAmount = quantity * unitPrice;

    const paidAmount = Number(
        purchaseRecordForm.paidAmount || 0
    );
    ...
    */
    ...
    if (!purchaseRecordForm.supplierRecordId) {
        notify(
        ...
        if (movement.transferType === "To Customer") {
            ...
            const nextRecords = units.map((unit, index) => {
                ...
                return {
                    ...
                    ownershipType:
                        movement.dealType === "Sold" ? "Sold" : "Loaned",
                    salePrice,
                    paidAmount,
  ...
  return;
        ...
        const hasInvalidRecordDeposit = selectedTransferIdentityRecords.some(
            (record) =>
                Number(transferForm.depositPrices[record.id] || 0) < 0
        );
  ...
  const customerTransferRecords = destinationUnitAssets.map((unit, index) => {
    ...
    const depositAmount =
      transferForm.dealType === "Loaned / Deposit"
        ? Number(
            isIndividualAsset
              ? transferForm.depositPrices[unitKey] ||
    ...
    return {
      ...
      ownershipType:
        transferForm.dealType === "Sold" ? "Sold" : "Loaned",
  ...
  if (!transfersSaved) return;
    ...
    if (!editMovementForm.supplierRecordId) {
        notify(
            "Please select a supplier.",
            "error"
        );
    ...
    if (
        ...
        notify(
            "Model, MAC Address and Serial Number are required for every device.",
            "error"
        );
        return;
    }
                            ...
                            <div className="asset-detail-form-grid">
                                ...
                                <label>
                                    ...
                                    <input
                                        type="number"
                                        min="0"
                                        value={purchaseRecordForm.unitPrice}
                                        onChange={(event) => {
                                ...
                                <label>
                                    Total Amount
                                    <input
                                        value={`${money(purchaseRecordTotalAmount)} AFN`}
                                        readOnly
                                        className="asset-detail-calculated-input"
                                    />
                                </label>
                                ...
                                <label>
                                    ...
                                    <input
                                        ...
                                        min="0"
                                        step="any"
                                        max={purchaseRecordTotalAmount}
                                        value={purchaseRecordForm.paidAmount || ""}
                                        onChange={(event) => {
                                            ...
                                            if (paidAmount > purchaseRecordTotalAmount) {
                                                notify(
                                                    `Paid amount cannot be greater than total amount (${money(purchaseRecordTotalAmount)} AFN).`,
                                                    "error"
                                                );

                                                setPurchaseRecordForm((previous) => ({
                                                    ...previous,
                                                    paidAmount: String(purchaseRecordTotalAmount),
                                                }));
                                                return;
                                            }
                                ...
                                <label>
                                    Remaining Amount
                                    <input
                                        value={`${money(purchaseRecordRemainingAmount)} AFN`}
                                        readOnly
                                        className="asset-detail-calculated-input"
                                    />
                                </label>
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        {transferForm.transferType === "To Customer" &&
                                                            ...
                                                            transferForm.dealType === "Sold" ? (
                                                            <div
                                                                className="asset-transfer-row-amounts sold"
                                                                onClick={(event) => event.stopPropagation()}
                                                            >
                                                                <input
                                                                    type="number"
                                    ...
                                    transferForm.dealType === "Sold" && (
                                        <div className="asset-transfer-sale-panel">
                                            ...
                                            <label>
                                                Total Remaining
                                                <input
                                                    value={`${money(customerRemainingTotal)} AFN`}
                                                    readOnly
                                                    className="asset-detail-calculated-input"
                                                />
                                            </label>
                ...
                >
                    ...
                    >
                        ...
                        <div className="asset-movement-detail-grid">
                            ...
                            <div><span>Source</span><strong>{viewMovement.sourceName || "-"}</strong></div>
                            <div><span>Destination</span><strong>{viewMovement.destinationName || "-"}</strong></div>
                            <div><span>Quantity</span><strong>{viewMovement.quantity || 0}</strong></div>
                            <div><span>Amount</span><strong>{money(viewMovement.totalAmount || viewMovement.estimatedLoss || viewMovement.trustAmount || 0)} AFN</strong></div>
                                ...
                                {(viewMovement.identityRecords || []).map((record, index) => (
                                    <div key={record.id || index}>
                                        <div>
                                            ...
                                            <small>
                                                Category: {record.category || asset?.category || "-"} / Unit Price:{" "}
                                                {money(record.unitPrice || asset?.unitPrice || 0)} AFN
                                            </small>
                                        </div>
                ...
                >
                    ...
                    >
                        ...
                        <div className="asset-movement-detail-grid">
                            ...
                            <div><span>Unit Price</span><strong>{money(viewMovementUnit.unitPrice || asset?.unitPrice || 0)} AFN</strong></div>
                            ...
                            <div className="asset-detail-form-grid">
                                ...
                                <label>
                                    ...
                                    <select
                                        ...
                                        disabled={repairResultForm.repairStatus === "Not Fixed"}
                                        onChange={(event) =>
                                            setRepairResultForm((previous) => ({
                                                ...previous,
                    ...
                    editMovementForm.dealType === "Sold" && (
                        <div className="asset-transfer-sale-panel">
                            ...
                            <label>
                                Total Remaining
                                <input
                                    value={`${money(editTransferRemainingTotal)} AFN`}
                                    readOnly
                                    className="asset-detail-calculated-input"
                                />
                            </label>
