...
function Accounts({ accounts, setAccounts, currentUser }) {
  ...
  const save = async (event) => {
    ...
    if (!form.email.trim()) return notify("لطفاً ایمیل را وارد کنید.", "error");
    if (!editId && !form.password) return notify("لطفاً رمز عبور را وارد کنید.", "error");
    ...
    let saved;
    if (editId) {
      saved = await setAccounts(accounts.map((item) => item.id === editId ? { ...item, fullName: form.fullName.trim(), email, ...(form.password ? { password: form.password } : {}) } : item));
      if (!saved) return;
  ...
  return <div className="accounts-page">
    ...
    {showModal && <div className="account-modal-backdrop" onClick={close}><div className="account-modal" onClick={(e) => e.stopPropagation()}><div className="account-modal-header"><div><h3>{editId ? "ویرایش اکونت" : "ساخت اکونت جدید"}</h3><p>این اکونت صلاحیت کامل خواهد داشت.</p></div><button onClick={close}>×</button></div><form onSubmit={save} noValidate><label>نام کامل<input value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} /></label><label>ایمیل<input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="name@example.com" /></label><label>{editId ? "رمز جدید (اختیاری)" : "رمز عبور"}<input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></label><label>تکرار رمز<input type="password" value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })} /></label><div><button type="button" onClick={close}>لغو</button><button type="submit">ذخیره اکونت</button></div></form></div></div>}
  </div>;
}
