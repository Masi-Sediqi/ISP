  ...
  return <div className="receipt-page">
    ...
    <article className="receipt-sheet financial-statement-sheet">
      <header className="receipt-header">
        ...
        <div className="receipt-number"><span>تاریخ صدور</span><strong>{formatAfghanDate(toDateValue(new Date()))}</strong><small>بازه: {period === "all" ? "تمام تاریخ‌ها" : `${formatAfghanDate(start)} تا ${formatAfghanDate(end)}`}</small></div>
      </header>
      ...
      <section className="receipt-section"><h3>تمام تعاملات مالی</h3><table className="receipt-history-table financial-statement-table"><thead><tr><th>تاریخ</th><th>حالت</th><th>عنوان</th><th>دسته‌بندی</th><th>منبع</th><th>مقدار</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td>{formatAfghanDate(item.date)}</td><td>{item.type === "income" ? "عاید" : "مصرف"}</td><td>{item.title || "-"}</td><td>{categoryLabel(item.category)}</td><td>{sourceLabel(item.source)}</td><td>{money(item.amount)}</td></tr>)}{items.length === 0 && <tr><td colSpan="6">ریکارد مالی وجود ندارد.</td></tr>}</tbody></table></section>
