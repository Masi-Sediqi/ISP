...
function Drivers() {
  ...
  const deleteEmployee = async (index) => {
    if (!window.confirm("آیا مطمئن هستید که این کارمند حذف شود؟")) return;
  ...
  return (
    <div className="drivers-page">
      ...
      {showModal && (
        <div className="driver-modal-backdrop" onClick={() => { resetForm(); setShowModal(false); }}>
          <div className="driver-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              <div className="driver-form-grid">
                <div className="form-group"><label>نام کارمند</label><input name="firstName" value={formData.firstName} onChange={handleChange} required /></div>
                <div className="form-group"><label>تخلص</label><input name="lastName" value={formData.lastName} onChange={handleChange} /></div>
                <div className="form-group"><label>شماره تماس</label><input name="phone" value={formData.phone} onChange={handleChange} required /></div>
                ...
                {formData.salaryType === "ثابت" && <div className="form-group"><label>مقدار معاش ثابت</label><input type="number" name="fixedSalary" value={formData.fixedSalary} onChange={handleChange} required /></div>}
                {formData.salaryType === "فیصدی" && <><div className="form-group"><label>فیصدی به اساس چی؟</label><select name="percentageBasis" value={formData.percentageBasis || "per_customer"} onChange={handleChange} required><option value="per_customer">به اساس فی مشتری</option><option value="per_trip">به اساس فی سفر</option><option value="monthly_income">به اساس عواید ماهانه</option></select></div><div className="form-group"><label>مقدار فیصدی</label><input type="number" name="percentage" value={formData.percentage} onChange={handleChange} required /></div></>}
                <div className="form-group">
                  <label>وضعیت</label>
      ...
      {showTypesModal && (
        <div className="driver-modal-backdrop" onClick={() => setShowTypesModal(false)}>
      ...
      {showPayrollModal && (
        <div className="driver-modal-backdrop" onClick={() => setShowPayrollModal(false)}>
          <div className="employee-types-modal" onClick={(event) => event.stopPropagation()}>
