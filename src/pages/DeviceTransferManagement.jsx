...
export default function DeviceTransferManagement() {
  ...
  const [rowDepositAmounts, setRowDepositAmounts] = useState({});
  const [rowRefundAmounts, setRowRefundAmounts] = useState({});
  const [rowRecoveryStatuses, setRowRecoveryStatuses] = useState({});
  const [rowCompanyOwesCustomer, setRowCompanyOwesCustomer] = useState({});
  ...
  const isCustomerSale = includesCustomerReceive && form.customerDeal === "Sold";
  const isCustomerDeposit = includesCustomerReceive && form.customerDeal === "Loaned / Deposit";
  ...
  const depositReceivedAmount = Number(form.depositReceivedAmount || 0);
  const refundAmount = Number(form.refundAmount || 0);
  const deductionAmount = Number(form.deductionAmount || 0);
  const saleAmount = Number(form.saleAmount || 0);
  const paidAmount = Number(form.paidAmount || 0);
  const depositReceiveRemaining = Math.max(depositAmount - depositReceivedAmount, 0);
  const refundRemainingDeposit = Math.max(sourceDepositHeld - refundAmount - deductionAmount, 0);
  ...
  const historyRows = useMemo(
    () => [...deviceHistory].sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0)),
    [deviceHistory]
  );
  ...
  const ledgerRowsForLocation = useMemo(() => {
    ...
    deviceTransfers.forEach((transfer) => {
      ...
      const key = [
        ...
        sourceType === "Customer" ? transfer.dealType || "unknown-deal" : "any-deal",
      ].join("::");
      const previous = grouped.get(key) || {
        ...
        ownershipType: transfer.dealType === "Sold" ? "Sold" : transfer.dealType ? "Loaned" : "",
        depositAmount: 0,
  ...
  const selectedDepositTotal = selectedAssetRows.reduce((sum, row) => {
    const quantity =
      row.tracking === "Individual"
        ? 1
        : Number(rowQuantities[row.rowKey] || row.defaultQuantity || 0);
    const defaultSalePrice = Number(row.asset.salePrice || row.asset.defaultSalePrice || row.asset.unitPrice || 0);
    const defaultDeposit = quantity * defaultSalePrice;
    return sum + Number(rowDepositAmounts[row.rowKey] ?? defaultDeposit);
  }, 0);
  const selectedAssetCostValue = selectedAssetRows.reduce((sum, row) => {
    const quantity =
      row.tracking === "Individual"
        ? 1
        : Number(rowQuantities[row.rowKey] || row.defaultQuantity || 0);
    return sum + quantity * Number(row.asset.unitPrice || 0);
  }, 0);
  ...
  const selectedRefundTotal = selectedAssetRows.reduce((sum, row) => {
    const held = Number(row.depositReceivedAmount || row.remainingDeposit || row.depositAmount || 0);
    return sum + Number(rowRefundAmounts[row.rowKey] ?? held);
  }, 0);
  ...
  const companyOwesCustomerTotal = selectedAssetRows.reduce((sum, row) => {
    if (!rowCompanyOwesCustomer[row.rowKey]) return sum;
    const held = Number(row.depositReceivedAmount || row.remainingDeposit || row.depositAmount || 0);
    return sum + Number(rowRefundAmounts[row.rowKey] ?? held);
  }, 0);
  ...
  const effectiveDepositAmount = isCustomerDeposit ? selectedDepositTotal : depositAmount;
  const effectiveDepositRemaining = Math.max(effectiveDepositAmount - depositReceivedAmount, 0);
  const effectiveRefundRemaining = Math.max(selectedDepositHeldTotal - selectedRefundTotal - deductionAmount, 0);
  ...
  const handleChange = (event) => {
    ...
    if (name === "transferType" || name === "sourceLocationId") {
      ...
      setRowQuantities({});
      setRowSalePrices({});
      setRowDepositAmounts({});
      setRowRefundAmounts({});
      setRowRecoveryStatuses({});
      setRowCompanyOwesCustomer({});
      setRowDamageAmounts({});
  ...
  const setRowSelected = (row, checked) => {
    ...
    if (checked) {
      ...
      const defaultSalePrice = Number(row.asset.salePrice || row.asset.defaultSalePrice || row.asset.unitPrice || 0);
      const heldDeposit = Number(row.depositReceivedAmount || row.remainingDeposit || row.depositAmount || 0);
      ...
      setRowDepositAmounts((previous) => ({
        ...previous,
        [row.rowKey]: String(previous[row.rowKey] ?? quantity * defaultSalePrice),
      }));
      setRowRefundAmounts((previous) => ({
        ...previous,
        [row.rowKey]: String(previous[row.rowKey] ?? heldDeposit),
      }));
  ...
  const deleteTransferRecord = async (transfer) => {
    const reversed = await reverseTransferEffects(transfer);
    if (!reversed) return;

    const transfersSaved = await setDeviceTransfers(
    ...
    if (!depositsSaved) return;

    const incomeRemoved = await removeTransferIncome(transfer);
    if (!incomeRemoved) return;

    const wasteExpenseRemoved = await removeTransferWasteExpense(transfer);
    if (!wasteExpenseRemoved) return;

    const damageIncomeRemoved = await removeTransferCustomerDamageIncome(transfer);
    if (!damageIncomeRemoved) return;

    notify("Transfer record deleted and stock was restored.");
  };
  ...
  const editTransferRecord = async (transfer) => {
    ...
    setForm({
      ...
      customerDeal: transfer.dealType || "Loaned / Deposit",
    ...
    notify("Transfer loaded into the form. Select the asset again, adjust values, and save.");
  };

  const nextTransferId = (offset = 0) =>
    `TRF-${String(deviceTransfers.length + offset + 1).padStart(5, "0")}`;
  ...
  const saveTransfer = async (event) => {
    ...
    if (isCustomerDeposit && depositReceivedAmount > effectiveDepositAmount) {
      notify("Received deposit cannot be greater than deposit amount.", "error");
      return;
    }

    if (includesCustomerRefund && selectedRefundTotal + deductionAmount > selectedDepositHeldTotal) {
      notify("Return amount and deduction cannot be greater than held deposit.", "error");
      return;
    }
    ...
    let remainingDepositReceivedToAllocate = depositReceivedAmount;
    ...
    const transferRecords = selectedAssetRows.map((row, index) => {
      ...
      const defaultDepositAmount = quantity * salePricePerQuantity;
      const rowDepositAmount = Number(rowDepositAmounts[row.rowKey] ?? defaultDepositAmount);
      const rowDepositReceivedAmount = isCustomerDeposit
        ? Math.min(rowDepositAmount, Math.max(remainingDepositReceivedToAllocate, 0))
        : 0;
      const rowPaidAmount = isCustomerSale
        ? Math.min(rowSaleAmount, Math.max(remainingSalePaidToAllocate, 0))
        : 0;
      const heldDeposit = Number(row.depositReceivedAmount || row.remainingDeposit || row.depositAmount || 0);
      const requestedReturnAmount = Number(rowRefundAmounts[row.rowKey] ?? heldDeposit);
      const companyOwesCustomer = Boolean(rowCompanyOwesCustomer[row.rowKey]);
      const rowRefundAmount = includesCustomerRefund && !companyOwesCustomer ? requestedReturnAmount : 0;
      const rowCustomerPayable = includesCustomerRefund && companyOwesCustomer ? requestedReturnAmount : 0;
      const rowDeductionAmount = includesCustomerRefund && !companyOwesCustomer
        ? Math.max(heldDeposit - requestedReturnAmount, 0)
        : 0;
      ...
      if (isCustomerDeposit) {
        remainingDepositReceivedToAllocate -= rowDepositReceivedAmount;
      }
      ...
      return {
        ...
        dealType: includesCustomerReceive ? form.customerDeal : row.ownershipType || "",
        ownershipType: includesCustomerReceive
          ? form.customerDeal === "Sold"
            ? "Sold"
            : "Loaned"
          : row.ownershipType || "",
        ...
        depositAmount: isCustomerDeposit ? rowDepositAmount : 0,
        depositReceivedAmount: isCustomerDeposit ? rowDepositReceivedAmount : 0,
        ...
        remainingDeposit: includesCustomerRefund
          ? rowCustomerPayable
          : isCustomerDeposit
            ? Math.max(rowDepositAmount - rowDepositReceivedAmount, 0)
            : 0,
        recoveryStatus: includesCustomerRefund ? rowRecoveryStatuses[row.rowKey] || "Fully Collected" : "",
        companyOwesCustomer,
    ...
    const nextAssets = assets.map((asset) => {
      ...
      const nextIdentityRecords = Array.isArray(asset.identityRecords)
        ? asset.identityRecords.map((record) => {
            ...
            return {
              ...
              customerRecordId: destinationType === "Customer" ? form.destinationLocationId : "",
              towerRecordId: destinationType === "Tower" ? form.destinationLocationId : "",
              updatedAt: createdAt,
            };
          })
        : asset.identityRecords;
      ...
      return {
        ...
        customerRecordId: shouldMoveWholeAsset && destinationType === "Customer" ? form.destinationLocationId : asset.customerRecordId || "",
        towerRecordId: shouldMoveWholeAsset && destinationType === "Tower" ? form.destinationLocationId : asset.towerRecordId || "",
        identityRecords: nextIdentityRecords,
        updatedAt: createdAt,
      };
    });
    ...
    const transfersSaved = await setDeviceTransfers([...deviceTransfers, ...transferRecords]);
    if (!transfersSaved) return;
    ...
    if (isCustomerDeposit) {
      const depositsSaved = await setSecurityDeposits([
        ...
        ...transferRecords.map((record) => ({
          ...
          receivedAmount: record.depositReceivedAmount,
          receivedDate: form.depositReceivedDate,
          paymentMethod: form.depositPaymentMethod,
          receiptNumber: form.depositReceiptNumber.trim(),
          receivedBy: form.depositReceivedBy.trim(),
          depositStatus: form.depositStatus,
          remainingDeposit: record.remainingDeposit,
          source: "device-transfer-management",
          createdAt,
          updatedAt: createdAt,
        })),
      ]);
    ...
    <div className="device-transfer-page">
      ...
      <form className="device-transfer-form device-transfer-form-modal" onSubmit={trySaveTransfer}>
        <div className="device-transfer-grid">
          ...
          <label>
            Transfer Date
            <input type="date" name="transferDate" value={form.transferDate} onChange={handleChange} required />
          </label>

          <label>
            Responsible User
            <input name="responsibleUser" value={form.responsibleUser} onChange={handleChange} required />
          </label>
          ...
          <label>
            Selected Quantity
            <input value={`${money(selectedTotalQuantity)} unit(s)`} readOnly className="device-transfer-readonly" />
          </label>
          ...
          {!includesCustomerReceive && (
            <div className="device-transfer-value-summary device-transfer-full">
              <div>
                <span>Selected Asset Cost Value</span>
                <strong>{money(selectedAssetCostValue)} AFN</strong>
              </div>
              {isCustomerWaste && (
                <>
                  <div>
          ...
          {includesCustomerReceive && (
            <div className="device-transfer-deposit-panel device-transfer-full">
              <div className="device-transfer-panel-title">
                <h3>Customer Deal</h3>
                <p>Use Sold for sales, or Loaned / Deposit for company-owned devices.</p>
              </div>
              <label>
                Deal Type
                <select name="customerDeal" value={form.customerDeal} onChange={handleChange}>
                  <option value="Loaned / Deposit">Loaned / Deposit</option>
              ...
              {isCustomerSale && (
                <>
                  <label>
                    Total Sale Amount
                    <input
                      value={`${money(effectiveSaleAmount)} AFN`}
                      readOnly
                      className="device-transfer-readonly"
                    />
                  </label>
                  <label>
                    Paid Amount
                    <input type="number" min="0" max={effectiveSaleAmount} name="paidAmount" value={form.paidAmount} onChange={handleChange} />
                  </label>
                  <label>
                    Remaining Amount
                    <input value={`${money(effectiveSaleRemaining)} AFN`} readOnly className="device-transfer-readonly" />
                  </label>
                </>
              )}
              ...
              {isCustomerDeposit && (
                <>
                  <label>
                    Deposit Amount
                    <input
                      value={`${money(effectiveDepositAmount)} AFN`}
                      readOnly
                      className="device-transfer-readonly"
                    />
                  </label>
                  <label>
                    Received Amount
                    <input type="number" min="0" max={effectiveDepositAmount} name="depositReceivedAmount" value={form.depositReceivedAmount} onChange={handleChange} />
                  </label>
                  <label>
                    Received Date
                  ...
                  <label>
                    Received By
                    <input name="depositReceivedBy" value={form.depositReceivedBy} onChange={handleChange} />
                  </label>
                  <label>
                    Deposit Status
                  ...
                  <label>
                    Remaining Deposit
                    <input value={`${money(effectiveDepositRemaining)} AFN`} readOnly className="device-transfer-readonly" />
                  </label>
                </>
              )}
            </div>
          ...
          {includesCustomerRefund && (
            <div className="device-transfer-deposit-panel device-transfer-full">
              <div className="device-transfer-panel-title">
                ...
                <p>Refund or deduct held deposit for returned customer devices.</p>
              </div>
              <label>
                Deposit Held
                <input value={`${money(selectedDepositHeldTotal)} AFN`} readOnly className="device-transfer-readonly" />
              </label>
              <label>
                Selected Return Amount
                <input value={`${money(selectedRefundTotal)} AFN`} readOnly className="device-transfer-readonly" />
              </label>
              <label>
                Company Owes Customer
                <input value={`${money(companyOwesCustomerTotal)} AFN`} readOnly className="device-transfer-readonly" />
              </label>
              <label>
                Remaining Deposit
                <input value={`${money(effectiveRefundRemaining)} AFN`} readOnly className="device-transfer-readonly" />
              </label>
              <label>
                Refund Date
          ...
          <label className="device-transfer-full">
            Reason
            <input name="reason" value={form.reason} onChange={handleChange} placeholder="Reason for transfer..." />
          </label>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  {isCustomerDeposit && <th>Deposit Amount</th>}
                  {includesCustomerRefund && <th>Recovery Status</th>}
                  {includesCustomerRefund && <th>Return Amount</th>}
                  {includesCustomerRefund && <th>Company Debt</th>}
              ...
              <tbody>
                {assetRows.map((row) => {
                  const sourceCustomerSold = sourceType === "Customer" && row.ownershipType === "Sold";
                  const rowSelectionDisabled = sourceCustomerSold;

                  return (
                  <tr
                    key={row.rowKey}
                    className={sourceCustomerSold ? "device-transfer-row-sold" : ""}
                  >
                    <td>
                      <input
                        type="checkbox"
                    ...
                    <td>{row.asset.assetId || "-"} - {row.asset.deviceName || "-"}</td>
                      ...
                      {row.tracking === "Individual" ? (
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            ...
                            setRowDepositAmounts((previous) => {
                              const nextQuantity = Math.min(Number(event.target.value || 1), row.availableQuantity);
                              const defaultSalePrice = Number(
                                row.asset.salePrice || row.asset.defaultSalePrice || row.asset.unitPrice || 0
                              );
                              return {
                                ...previous,
                                [row.rowKey]: String(nextQuantity * defaultSalePrice),
                              };
                            })
                          }
                        />
                    ...
                    {includesCustomerRefund && (
                      <td>
                        <input
                          type="number"
                          min="0"
                          max={Number(row.depositReceivedAmount || row.remainingDeposit || row.depositAmount || 0)}
                          value={
                            rowRefundAmounts[row.rowKey] ??
                            Number(row.depositReceivedAmount || row.remainingDeposit || row.depositAmount || 0)
                          }
                          disabled={!selectedRows[row.rowKey] || rowSelectionDisabled}
                          onChange={(event) =>
                            setRowRefundAmounts((previous) => ({
                    ...
                    {includesCustomerRefund && (
                      <td>
                        <button
                          type="button"
                          className={`device-transfer-debt-toggle ${
                            rowCompanyOwesCustomer[row.rowKey] ? "active" : ""
                          }`}
                          disabled={!selectedRows[row.rowKey] || rowSelectionDisabled}
                          onClick={() =>
                            setRowCompanyOwesCustomer((previous) => ({
                              ...previous,
                              [row.rowKey]: !previous[row.rowKey],
                            }))
                          }
                        >
                          Make Company Owe
                  ...
                  <tr>
                    <td
                      colSpan={
                        10 +
                        (isCustomerSale ? 1 : 0) +
                        (isCustomerDeposit ? 1 : 0) +
                        (includesCustomerRefund ? 3 : 0) +
                        (isCustomerWaste ? 2 : 0)
                      }
      ...
      <section className="device-transfer-card">
        <div className="device-transfer-card-header">
          ...
          <p>Records can be edited or deleted here; Device History remains as the audit trail.</p>
        </div>
        <div className="device-transfer-table-wrap">
          <table>
            <thead>
            ...
            <tbody>
              {transferRows.map((transfer) => (
                <tr key={transfer.id}>
                  ...
                  <td>{transfer.assetId || "-"} - {transfer.deviceName || "-"}</td>
                  ...
                  <td>{transfer.dealType || "-"}</td>
                  <td>{transfer.depositAmount ? `${money(transfer.depositReceivedAmount || 0)} / ${money(transfer.depositAmount)} AFN` : "-"}</td>
                  <td>{transfer.paidAmount ? `${money(transfer.paidAmount)} AFN` : "-"}</td>
                  <td>{transfer.previousStatus || "-"}</td>
                  <td>{transfer.newStatus || "-"}</td>
                  ...
                  <td>
                    <div className="device-transfer-row-actions">
                      ...
                      <button type="button" className="danger" onClick={() => deleteTransferRecord(transfer)}>
                        Delete
                      </button>
                    </div>
                  </td>
              ...
              {transferRows.length === 0 && (
                <tr>
                  <td colSpan="18" className="device-transfer-empty">No transfer has been recorded yet.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <section className="device-transfer-card">
        <div className="device-transfer-card-header">
          ...
          <p>Immutable history generated by each transfer.</p>
        </div>
        <div className="device-transfer-table-wrap">
          <table>
            <thead>
            ...
            <tbody>
              {historyRows.map((history) => (
                <tr key={history.id}>
                  ...
                  <td>{history.assetId || "-"} - {history.deviceName || "-"}</td>
                  <td>
                    {history.sourceLocation || "-"} {"->"} {history.destinationLocation || "-"}
                  </td>
                  <td>{money(history.quantity)} {history.unit || ""}</td>
                  <td>{history.previousStatus || "-"}</td>
                  <td>{history.newStatus || "-"}</td>
                  <td>{formatDateTime(history.createdDate || history.createdAt)}</td>
                </tr>
              ))}
              {historyRows.length === 0 && (
                <tr>
                  <td colSpan="8" className="device-transfer-empty">No device history has been recorded yet.</td>
                </tr>
              )}
            </tbody>
          </table>
