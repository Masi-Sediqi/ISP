...
export default function Repair() {
  ...
  const stats = useMemo(() => {
    ...
    return { pending, fixed, notFixed, cost };
  }, [repairRows]);
  ...
  const openResultModal = (transfer) => {
    ...
    setResultForm({
      ...
      customerDeal: result.customerDeal || transfer.ownershipType || "Loaned / Deposit",
  ...
  const saveRepairResult = async (event) => {
    ...
    const quantity = Number(resultForm.quantity || 0);
    const maxQuantity = Number(resultTransfer.quantity || 0);
    const repairCost = Number(resultForm.repairCost || 0);
    const salePricePerQuantity = Number(resultForm.salePricePerQuantity || 0);
    const totalAmount = quantity * salePricePerQuantity;
    const paidAmount = Number(resultForm.paidAmount || 0);
    const depositAmount = Number(resultForm.depositAmount || 0);
    const depositReceivedAmount = Number(resultForm.depositReceivedAmount || 0);
    ...
    if (["Tower", "Customer"].includes(resultForm.sendTo) && !resultForm.destinationId) {
      notify(`Please select a ${resultForm.sendTo}.`, "error");
      return;
    }

    if (resultForm.sendTo === "Customer" && resultForm.customerDeal === "Sold") {
      if (!Number.isFinite(salePricePerQuantity) || salePricePerQuantity < 0) {
    ...
    if (resultForm.sendTo === "Customer" && resultForm.customerDeal === "Loaned / Deposit") {
      if (depositReceivedAmount > depositAmount) {
        notify("Deposit received cannot be greater than deposit amount.", "error");
        return;
      }
    }
    ...
    const result = {
      ...
      sendTo: resultForm.sendTo,
      destinationRecordId: resultForm.destinationId,
      destinationLocation: locationName(resultForm.sendTo, resultForm.destinationId, towers, customers),
      customerDeal: resultForm.sendTo === "Customer" ? resultForm.customerDeal : "",
      ownershipType:
        resultForm.sendTo === "Customer"
          ? resultForm.customerDeal === "Sold"
            ? "Sold"
            : "Loaned"
          : "",
      salePricePerQuantity: resultForm.sendTo === "Customer" ? salePricePerQuantity : 0,
      totalAmount: resultForm.sendTo === "Customer" && resultForm.customerDeal === "Sold" ? totalAmount : 0,
      salePrice: resultForm.sendTo === "Customer" && resultForm.customerDeal === "Sold" ? totalAmount : 0,
      paidAmount: resultForm.sendTo === "Customer" && resultForm.customerDeal === "Sold" ? paidAmount : 0,
      remainingAmount:
        resultForm.sendTo === "Customer" && resultForm.customerDeal === "Sold"
          ? Math.max(totalAmount - paidAmount, 0)
          : 0,
      depositAmount:
        resultForm.sendTo === "Customer" && resultForm.customerDeal === "Loaned / Deposit"
          ? depositAmount
          : 0,
      depositReceivedAmount:
        resultForm.sendTo === "Customer" && resultForm.customerDeal === "Loaned / Deposit"
      ...
      remainingDeposit:
        resultForm.sendTo === "Customer" && resultForm.customerDeal === "Loaned / Deposit"
          ? Math.max(depositAmount - depositReceivedAmount, 0)
          : 0,
      depositStatus:
        resultForm.sendTo === "Customer" && resultForm.customerDeal === "Loaned / Deposit"
          ? resultForm.depositStatus
          : "",
      responsibleUser: resultForm.responsibleUser.trim(),
  ...
  return (
    <div className="repair-page">
      ...
      <section className="repair-stats">
        <div><span>Total Repair Records</span><strong>{repairRows.length}</strong><p>Assets sent to repair</p></div>
        <div><span>Pending Result</span><strong>{stats.pending}</strong><p>Waiting for repair result</p></div>
        <div><span>Fixed</span><strong>{stats.fixed}</strong><p>Successfully repaired</p></div>
        <div><span>Not Fixed</span><strong>{stats.notFixed}</strong><p>Repair failed or unavailable</p></div>
        <div><span>Repair Expenses</span><strong>{money(stats.cost)} AFN</strong><p>Linked to Financial</p></div>
      </section>
      ...
      {resultTransfer && (
        <div className="repair-modal-backdrop" onMouseDown={(event) => {
          if (event.target === event.currentTarget) closeResultModal();
        }}>
              ...
              <div className="repair-form-grid">
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) => setResultForm((previous) => ({ ...previous, repairStatus: event.target.value }))}
                  >
                    <option value="Fixed">Fixed</option>
                    <option value="Not Fixed">Not Fixed</option>
                  </select>
                ...
                <label>
                  Quantity ({resultTransfer.unit || getAssetUnit(assets.find((asset) => keyOf(asset.assetId) === keyOf(resultTransfer.assetId)))})
                  <input
                    type="number"
                    min="1"
                    max={Number(resultTransfer.quantity || 1)}
                    value={resultForm.quantity}
                    onChange={(event) => setResultForm((previous) => ({ ...previous, quantity: event.target.value }))}
                    required
                  />
                </label>
                ...
                {resultForm.sendTo === "Customer" && (
                  <>
                    ...
                    <label>
                      Customer Deal
                      <select
                        value={resultForm.customerDeal}
                        onChange={(event) => setResultForm((previous) => ({ ...previous, customerDeal: event.target.value }))}
                      >
                    ...
                    <label>
                      ...
                      <input
                        type="number"
                        min="0"
                        value={resultForm.salePricePerQuantity}
                        onChange={(event) => setResultForm((previous) => ({ ...previous, salePricePerQuantity: event.target.value }))}
                    ...
                    {resultForm.customerDeal === "Sold" ? (
                      <>
                        <label>
                          Total Sale Amount
                          <input value={`${money(Number(resultForm.quantity || 0) * Number(resultForm.salePricePerQuantity || 0))} AFN`} readOnly />
                        </label>
                        <label>
                          Paid Amount
                          <input
                            type="number"
                            min="0"
                            max={Number(resultForm.quantity || 0) * Number(resultForm.salePricePerQuantity || 0)}
                            value={resultForm.paidAmount}
                            onChange={(event) => setResultForm((previous) => ({ ...previous, paidAmount: event.target.value }))}
                          />
                        </label>
                        <label>
                          Remaining Amount
                          <input value={`${money(Math.max(Number(resultForm.quantity || 0) * Number(resultForm.salePricePerQuantity || 0) - Number(resultForm.paidAmount || 0), 0))} AFN`} readOnly />
                        </label>
                      </>
                    ) : (
                      <>
                        ...
                        <label>
                          ...
                          <input
                            type="number"
                            min="0"
                            max={Number(resultForm.depositAmount || 0)}
                            value={resultForm.depositReceivedAmount}
                            onChange={(event) => setResultForm((previous) => ({ ...previous, depositReceivedAmount: event.target.value }))}
                          />
                        </label>
                        <label>
                          Deposit Status
                ...
                <label>
                  Responsible User
                  <input
                    value={resultForm.responsibleUser}
                    onChange={(event) => setResultForm((previous) => ({ ...previous, responsibleUser: event.target.value }))}
                  />
                </label>
