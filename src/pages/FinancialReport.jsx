  ...
  return (
    <div className="reports-page financial-report-page">
      ...
      <div className="financial-performance-grid">
        ...
        <section className="financial-ranking-card"><div className="financial-ranking-title"><span>راپور مالی مشتری‌ها</span><h3>پرداخت و بدهی مشتریان</h3></div>{customerData.slice(0, 6).map((customer, index) => <Link to="/customers/consultants" key={customer.customerIndex}><b>{index + 1}</b><div><strong>{customer.name}</strong><span>پرداخت {money(customer.paid)} · تخفیف {money(customer.discount)}</span></div><em className={customer.debt > 0 ? "negative" : "positive"}>{money(customer.debt)}</em></Link>)}{customerData.length === 0 && <p className="financial-empty">ریکارد مشتری در این بازه وجود ندارد.</p>}</section>
      </div>
      ...
      <div className="financial-alert-grid">
        ...
        <section className="financial-ranking-card debt-list"><div className="financial-ranking-title"><span>بدهی مشتری‌ها</span><h3>طلب‌های قابل دریافت</h3></div>{largestDebtors.map((customer, index) => <Link to="/customers/consultants" key={customer.customerIndex}><b>{index + 1}</b><div><strong>{customer.name}</strong><span>کرایه خالص {money(customer.billed)} · پرداخت {money(customer.paid)}</span></div><em>{money(customer.debt)}</em></Link>)}{largestDebtors.length === 0 && <p className="financial-empty">هیچ بدهی‌ای در این بازه وجود ندارد.</p>}</section>
      </div>
