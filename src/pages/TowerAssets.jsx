...
const filteredAssets = buildAvailableAssetOptions().filter((asset) => {
  ...
  const totalRecords = towerAssets.length;
  const installedRecords = towerAssets.filter((item) => item.installationStatus === "Installed").length;
  const pendingRecords = towerAssets.filter((item) => item.installationStatus === "Pending").length;
  const maintenanceRecords = towerAssets.filter((item) => item.installationStatus === "Maintenance").length;

  const uniqueTowers = new Set(
    towerAssets.map((item) => item.towerName).filter(Boolean)
  ).size;
...
const openEditModal = (index) => {
    ...
    <div className="tower-page">
      ...
      <div className="tower-stats">
        <div className="tower-stat-card">
          ...
          <p>All tower asset records</p>
        </div>

        <div className="tower-stat-card">
          <span>Total Towers</span>
          <strong>{uniqueTowers}</strong>
          <p>Unique towers with assets</p>
        </div>

        <div className="tower-stat-card">
          <span>Installed</span>
          <strong>{installedRecords}</strong>
          <p>Installed tower devices</p>
        </div>

        <div className="tower-stat-card">
          <span>Pending</span>
          <strong>{pendingRecords}</strong>
          <p>Pending installation records</p>
        </div>

        <div className="tower-stat-card">
          <span>Maintenance</span>
          <strong>{maintenanceRecords}</strong>
          <p>Devices under maintenance</p>
        </div>
      </div>
  ...
  {towerPagination.pageItems.map((record) => {
    ...
    const recordAssets = getRecordAssets(record);
      ...
      <tr key={record.id || index}>
        ...
        <td>
          <span className="tower-assets-count">
            {recordAssets.length} asset{recordAssets.length === 1 ? "" : "s"}
          </span>
        </td>
        ...
        <td>
          {Number(record.installationCost || 0).toLocaleString("en-US")} AFN
        </td>
  ...
  {filteredTowerAssets.length === 0 && (
    <tr>
      ...
      {showModal && (
        <div className="tower-modal-backdrop" onClick={closeModal}>
          <div className="tower-modal" onClick={(event) => event.stopPropagation()}>
            <div className="tower-modal-header">
              <div>
                <h3>{editIndex !== null ? "Edit Tower Asset" : "Add Tower Asset"}</h3>
                <p>Enter tower, device, issue date, and installation information.</p>
              </div>
    ...
    <div className="tower-detail-modal" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="tower-detail-grid">
        ...
        <div>
          ...
          <strong>
            {Number(detailRecord.installationCost || 0).toLocaleString("en-US")} AFN
          </strong>
        </div>
          ...
          <table>
            ...
            <tbody>
              ...
              {getRecordAssets(detailRecord).length === 0 && (
                <tr>
                  <td colSpan="8" className="tower-empty">
