...
export async function createCompleteBackup({ backupType = "manual" } = {}) {
  if (!serverConfigured) throw new Error("VPS server is not configured.");
  if (!navigator.onLine) throw new Error("Internet is required to create a VPS server backup.");
...
async function restoreLegacyBackup(parsed) {
  ...
  if (oldCompleteRows) {
    if (!serverConfigured || !navigator.onLine) {
      throw new Error("Internet and VPS server configuration are required to restore this backup.");
    }
  ...
  if (!serverConfigured || !navigator.onLine) {
    throw new Error("Internet and VPS server configuration are required to restore backup data.");
  }
